...
class Host:
    ...
    def udt_send(self, dst_addr, data_S):
        ...
        if(len(data_S) > self.out_intf_L[0].mtu):
            # dist_addr is length 5, subtract from mtu to get the proper packet length limit
            l = self.out_intf_L[0].mtu - 5
            # break into two packets
            p1 = NetworkPacket(dst_addr, data_S[0:l])
            p2 = NetworkPacket(dst_addr, data_S[l:])
            # send the packets
            self.out_intf_L[0].put(p1.to_byte_S())
            print('host sending packet "%s" with mtu=%d' % (p1, self.out_intf_L[0].mtu))
            self.out_intf_L[0].put(p2.to_byte_S())
            print('host sending packet "%s" with mtu=%d' % (p2, self.out_intf_L[0].mtu))
